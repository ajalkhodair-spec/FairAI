...
- Recovery latency: time from publisher restart initiation until a newly
  published artifact is retrieved and byte-verified by the consumer.
...
The clean run `ipfs-native-pin-cold-30rep` contains 150 sequential and 120
...
artifact retrievals. The clean run `ipfs-recovery-30rep-6ba3a9c` adds 30
publisher process outage/restart trials with identity-stability checks. Consumer
outage, filesystem comparison, WAN, and multi-host measurements remain
unavailable; no local hash timing is substituted for them.
